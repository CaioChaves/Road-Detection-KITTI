...
import os
from model_archicteture import MyNetwork, MinimalisticCNN
...
def train(args, model, device, train_dataloader, optimizer, epoch, patch_size, ppi, loss_func):
    ...
    for _, (original, segmentation) in enumerate(train_dataloader):
        for _ in range(ppi):
            patch_x_pos = random.randint(0,original.shape[3]-patch_size)
            patch_y_pos = random.randint(0,original.shape[2]-patch_size)
...
def main():
    ...
    if args.model_arch == 'base':
        ...
        model = MinimalisticCNN(in_channels=3,num_classes_out=1,spatial_features=args.spatial_feats).to(device)
    ...
    #ppi = int(args.ppi_coeff*images.shape[2]*images.shape[3]/(args.patch_size*args.patch_size))      # Non-dimensional parameter (approx. 575)
    ppi = args.ppi
